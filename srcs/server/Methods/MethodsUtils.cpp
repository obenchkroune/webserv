...
std::pair<HttpStatus, std::string> ServerClient::ProcessFilePermission(const Request    &request,
                                                                       LocationsIterator file_location, int permission)
{
    if (std::find(file_location->allow_methods.begin(), file_location->allow_methods.end(), request.getMethod()) ==
        ...
        return std::pair<HttpStatus, std::string>(HttpStatus(STATUS_METHOD_NOT_ALLOWED, HTTP_STATUS_METHOD_NOT_ALLOWED),
                                                  "");
    ...
    if (ServerUtils::validateFileLocation(file_location->root, file_name) == false)
    {
        ...
        std::cerr << "Access for file: " << file_name << " is outside the location root, request is forbidden."
                  << std::endl;
        return std::pair<HttpStatus, std::string>(HttpStatus(STATUS_FORBIDDEN, HTTP_STATUS_FORBIDDEN), "");
    }
    if (access(file_name.c_str(), F_OK) != 0) // EXISTENCE ACCESS
        return std::pair<HttpStatus, std::string>(HttpStatus(STATUS_NOT_FOUND, HTTP_STATUS_NOT_FOUND), "");
