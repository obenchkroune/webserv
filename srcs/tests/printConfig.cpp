...
void printServerConfig(const ServerConfig &config)
{
    ...
    for (std::vector<std::string>::const_iterator it = config.server_names.begin(); it != config.server_names.end();
         ++it)
    {
        std::cout << *it << " ";
    }
    ...
    for (std::vector<HttpMethod>::const_iterator it = config.allow_methods.begin(); it != config.allow_methods.end();
         ++it)
    {
        switch (*it)
        {
    ...
    std::cout << "\nLocations:" << std::endl;
    for (std::vector<LocationConfig>::const_iterator loc = config.locations.begin(); loc != config.locations.end();
         ++loc)
    {
        ...
        std::cout << "    Allowed Methods: ";
        for (std::vector<HttpMethod>::const_iterator it = loc->allow_methods.begin(); it != loc->allow_methods.end();
             ++it)
        {
            switch (*it)
            {
        ...
        if (loc->redirect)
        {
            std::cout << "    Redirect: " << loc->redirect_code << " -> " << loc->redirect_path << std::endl;
        }
